...
async function carregarRanking() {
    ...
    try {
        const { data, error } = await window.supabaseClient
            .from('profiles')
            .select('username, pontos_totais, level, avatar_url, equipped_name_style')
        ...
        data.forEach((profile, index) => {
            ...
            row.innerHTML = `
                ...
                <div class="rank-col rank-user">
                    ${avatarHtml}
                    <!-- O span do username será inserido aqui pelo JS -->
                </div>
            ...
            `;
            row.querySelector('.rank-user').appendChild(usernameSpan);
