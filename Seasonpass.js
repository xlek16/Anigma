...
function gerarRecompensas() {
  ...
  for (let lv = 1; lv <= 75; lv++) {
    ...
    else if (lv === 5)  { free = { d:25 };                                         prem = { d:80 }; }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('spMain').style.display = 'block';
...
function capType(t) { return { avatar:'Avatar', title:'Título', style:'Estilo', frame:'Frame' }[t] || t; }
...
function processarSpecial(special, cosmeticUpdates) {
  ...
  if (special.type === 'title') {
    ...
    if (!arr.includes(special.val)) arr.push(special.val);
    cosmeticUpdates.unlocked_avatars = arr;
  }
}
