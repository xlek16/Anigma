...
async function carregarProgresso() {
  try {
    ...
    const { data } = await window.supabaseClient
      .from('profiles')
      .select('diamantes, pontos_totais, pontos_jjk, tentativas_jjk, jogo_terminado_jjk, ultimo_jogo_jjk, palpites_jjk, claim_feito_jjk')
    ...
    if (data.ultimo_jogo_jjk === hoje) {
      ...
      estado.claimFeito    = data.claim_feito_jjk    ?? false;
      ...
      await window.supabaseClient.from('profiles').update({
        ...
        claim_feito_jjk:    false,
        ultimo_jogo_jjk:    hoje,
        palpites_jjk:       '[]',
      }).eq('id', session.user.id);
    }
