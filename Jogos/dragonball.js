...
async function carregarProgresso() {
  try {
    ...
    const { data } = await window.supabaseClient
      .from('profiles')
      .select('diamantes, pontos_totais, pontos_db, tentativas_db, jogo_terminado_db, ultimo_jogo_db, palpites_db, claim_feito_db')
    ...
    if (data.ultimo_jogo_db === hoje) {
      ...
      estado.jogoTerminado = data.jogo_terminado_db ?? false;
      estado.claimFeito    = data.claim_feito_db    ?? false;
      ...
      await window.supabaseClient.from('profiles').update({
        ...
        tentativas_db:  0,
        jogo_terminado_db: false,
        claim_feito_db:    false,
        ultimo_jogo_db:    hoje,
        palpites_db:    '[]',
      }).eq('id', session.user.id);
    }
...
function mostrarBotaoClaim(diamantesGanhos) {
  ...
  btn.onclick = async function () {
    ...
    try {
      // 1. Obter perfil atual para calcular o streak corretamente
      const { data: currentProfile, error: profileError } = await window.supabaseClient
        .from('profiles')
        .select('current_streak, last_streak_date, pontos_totais, diamantes, unlocked_achievements, level')
        .eq('id', sessaoAtiva.user.id)
        .single();
      ...
      const lastStreakDate = currentProfile.last_streak_date;

      if (lastStreakDate !== hojeStr) { // Apenas atualiza o streak se ainda não foi atualizado hoje
        if (lastStreakDate === ontemStr) {
          newStreak++; // Jogou ontem, incrementa
        } else {
          newStreak = 1; // Falhou um dia ou é a primeira vez, reseta para 1
        }
      }

      const novosDiamantes = (currentProfile.diamantes || 0) + diamantesGanhos;
      const novosPontos    = (currentProfile.pontos_totais || 0) + pontosFinais;
      ...
      if (typeof handleLevelUp === 'function') {
        await handleLevelUp(sessaoAtiva.user.id, currentProfile.pontos_totais, novosPontos);
      }

      // Verificar conquistas
      if (typeof verificarConquistas === 'function') {
        // Criar objeto perfil temporário para verificação
        const perfilTemp = { ...updatedProfile };
